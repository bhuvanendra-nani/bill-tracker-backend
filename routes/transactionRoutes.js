...
function buildPhotoUrl(req, filename) {
  return `${req.protocol}://${req.get("host")}/uploads/transactions/${filename}`;
}
...
router.post("/", authMiddleware, upload.single("photo"), async (req, res) => {
  try {
    const { title, amount, type, category, date, note, photoUrl } = req.body;
    ...
    if (req.file) {
      photo = buildPhotoUrl(req, req.file.filename);
    } else if (photoUrl) {
      photo = photoUrl;
    }
...
router.put("/:id", authMiddleware, upload.single("photo"), async (req, res) => {
  try {
    ...
    let photo = existingTransaction.photo || "";
    ...
    existingTransaction.title = title ?? existingTransaction.title;
    ...
    existingTransaction.note = note ?? existingTransaction.note;
    existingTransaction.photo = photo;

    await existingTransaction.save();

    return res.json({ transaction: existingTransaction.toJSON() });
  } catch (error) {
    return res.status(500).json({
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      existingTransaction.voiceNote = `${req.protocol}://${req.get("host")}/uploads/voice-notes/${req.file.filename}`;
      ...
      return res.status(500).json({
        message: error.message || "Failed to attach voice note",
      });
    }
  }
);
